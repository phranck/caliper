...
def write_header(data: dict) -> None:
    ...
    # Through the formatter the repository holds every other header to, rather
    # than exempting this one. A table of layouts wraps at no line a person
    # would choose, and the style check is what would otherwise find that out
    # after the push.
    try:
        body = subprocess.run(["clang-format", f"--assume-filename={HEADER.name}"],
                              input=body, capture_output=True, text=True, check=True).stdout
    except FileNotFoundError:
        print("  clang-format is not here, so the header goes out unformatted")
